...
$settings = @{}
$(dig +short laboratory.environment.private txt).Split("\n") | % { 
...
$runsQueue = $settings["runsQueue"]

# Configure storage
az storage queue create --auth-mode login --account-name $storageAccount -n $runsQueue
